...
/** Keys the type filter accepts. Most map 1:1 to a `Conversation["type"]`; `channels` groups both kinds. */
export type TypeFilter = "im" | "mpim" | "channels" | "channel" | "private_channel";
...
const CONVERSATION_TYPES: {
  ...
  { value: "im", title: "DMs", icon: Icon.Person, color: Color.SecondaryText, types: ["im"] },
  { value: "mpim", title: "Group DMs", icon: Icon.TwoPeople, color: Color.Yellow, types: ["mpim"] },
  {
    value: "channels",
    title: "Channels",
    icon: Icon.Hashtag,
    color: Color.Blue,
    types: ["channel", "private_channel"],
  },
  { value: "channel", title: "Public Channels", icon: Icon.Hashtag, color: Color.Blue, types: ["channel"] },
  {
...
function matchesFilter(conversation: Conversation, filter: string, convTags: Record<string, string[]>): boolean {
  ...
  if (filter.startsWith(TYPE_PREFIX)) {
    const key = filter.slice(TYPE_PREFIX.length);
    const entry = CONVERSATION_TYPES.find((t) => t.value === key);
    return entry ? entry.types.includes(conversation.type) : true;
  }
...
export type ConversationsListProps = {
  /** Pre-selects this type in the filter dropdown. The user can still change it. */
  initialType?: TypeFilter;
};

export default function ConversationsList({ initialType }: ConversationsListProps = {}) {
  const initialEntry = initialType ? CONVERSATION_TYPES.find((t) => t.value === initialType) : undefined;
  ...
  const [filter, setFilter] = useState<string>(initialType ? `${TYPE_PREFIX}${initialType}` : FILTER_ALL);
  ...
  return (
    <List
      ...
      searchBarPlaceholder={`Search ${initialEntry ? initialEntry.title.toLowerCase() : "conversations"}...`}
